...
Four classes for the tickets, so the picture says what the frontier says:
closed, takeable now, claimed, blocked. The root wears its own class — the
map is not a ticket, and painting it `claimed` said it was one. A blocker
that is not a ticket of this map gets its own node marked external, because
a dangling edge to nothing is the one thing a diagram must not do.
...
def _class_of(child, frontier_keys) -> str:
...
def run(params: dict, *, tracker) -> str:
    ...
    lines = ["```mermaid", "flowchart TD", f'    {_node_id(key)}["{_label(key)} — the map"]']
    for child in children:
        label = f"{child.key} — {_label(child.title)}"
        if child.assignee and not S.is_closed(child):
            label += f" ({_label(child.assignee)})"
        lines.append(f'    {_node_id(child.key)}["{label}"]')
        lines.append(f"    {_node_id(key)} --- {_node_id(child.key)}")
    ...
    for blocker in external:
        lines.append(f'    {_node_id(blocker)}["{blocker} — external to this map"]')

    for child in children:
        for blocker in child.blocked_by:
            lines.append(f"    {_node_id(blocker)} --> {_node_id(child.key)}")

    lines += list(_CLASSES)
    lines.append(f"    class {_node_id(key)} root")
    for child in children:
        lines.append(f"    class {_node_id(child.key)} {_class_of(child, frontier_keys)}")
    for blocker in external:
        lines.append(f"    class {_node_id(blocker)} external")
    lines.append("```")
