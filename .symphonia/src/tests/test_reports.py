"""Golden tests for the report parsers in `gate`.

TLDR: the filled examples in `roles/planner.md` (the `io:example-*` fenced
blocks) are the golden fixtures — they parse back into the fields a human
typed, and a body that drifts from the contract raises `MalformedReport`
naming what is wrong. Run either way:

    cd .symphonia/src && python3 -m unittest tests.test_reports
    python3 .symphonia/src/tests/test_reports.py
"""
...
class TestPlanSubmissionProseRequiresTheLocalTechnicalPlanSection(unittest.TestCase):
    """The example alone showed `## Local Technical Plan` — a planner who
    read the prose and skipped the example could submit a body without it,
    and `verdict()` publishes that section verbatim on approval. The prose
    must require it too, not just the example."""
...
class TestFormatApprovalReplyRoundTrips(unittest.TestCase):
    """`spawn verdict` formats through this instead of hand-assembling the
    token/notes shape — round-trips through `parse_approval_reply`."""
