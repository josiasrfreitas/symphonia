...
- Start every role through `.symphonia/bin/spawn` and hear back through one `check --wait` loop — read `.symphonia/bin/README.md` before your first spawn. You choose no model, no permission flag and no launch path; they are decided in `.symphonia/adapters/orca/launcher.py`.
